# Plotting logistic regressions with marginal distributions of Y -- v4
#
...
#
# Version 4 is an alternative grouped-density treatment for visual comparison with version 3.
# It keeps the same density estimates, normalization, lane ordering, and public interface, but
# uses very thin, horizontally symmetric grouped-density strips resembling wedges.
# Version 3 remains unchanged as the wider-strip alternative.
#
...
#' Grouped densities are drawn as thin, symmetric strips centered on horizontal axes in lanes
#' immediately outside 0 and 1, then stacked outward. Each group's two response-specific
#' densities are normalized together within its lane, so the shapes show conditional
#' distributions but do not encode group sample sizes.
#'
...
#'
#' # grouped fits and marginals; symmetric density strips stack outside 0 and 1
...
.logist_plot_impl <- function(x, y, marginal = c("hist", "points", "density"),
  ...
  if (marginal == "points") {
    ...
    if (grouped) {
      # Thin-strip alternative: retain the density shape but cap each lane at 3% of the
      # original 0--1 response-panel height. Keep the inter-lane gap and outer stroke padding
      # at their version-3 absolute sizes so the narrow strips remain clearly separated.
      density_lane_height <- 0.03
      density_lane_gap <- 0.01
      ...
      # panel border; this does not change the density estimates or configured lane height.
      density_outline_padding <- 0.0075
      ...
      make_group_density_data <- function(group_index, response) {
        ...
        if (response == 0) {
          centerline <- -density_lane_height / 2 - lane_offset
        } else {
          centerline <- 1 + density_lane_height / 2 + lane_offset
        }
        # Draw a thin violin-like strip centered on one horizontal axis. Local thickness is
        # proportional to the density and is divided equally above and below the centerline.
        ymin <- pmax(centerline - scaled_density / 2, density_limits[1L])
        ymax <- pmin(centerline + scaled_density / 2, density_limits[2L])
        data.frame(
          x = estimate$x,
          ymin = ymin,
          ymax = ymax,
      ...
      density_plot <- p_base +
        grouped_density_layer(density_y0, "both") +
        grouped_density_layer(density_y1, "both") +
        fit_layer +
        ggplot2::scale_y_continuous(
