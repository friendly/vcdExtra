...
#
# 🔧 WIP: [HARD] When there are two (or more) variables for the row, these should appear as a nested
...
#         example.
#         Attempt below uses gt's multi-column stub support (`rowname_col` accepting a
#         character vector of column names, gt >= 0.11) -- see dev/color_table/multi-col-stubs.md
#         and dev/color_table/test-multi-col-stubs.R. Mirrors the existing has_multi_col_vars
#         spanner logic below, using the `row_vars` attribute (attached by the S3 methods but
#         previously unused).
#
