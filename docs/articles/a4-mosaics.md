...
- **Symmetry**: This model asserts that the joint distribution of the
  row and column variables is symmetric, that is
  $`\pi_{ij} = \pi_{ji}`$: A son is equally likely to move from their
  father’s occupational category $`i`$ to another category, $`j`$, as
  the reverse, moving from $`j`$ to $`i`$. Symmetry is quite strong,
  because it also implies **marginal homogeneity**, that the marginal
  probabilities of the row and column variables are equal,
  $`\pi{i+} = \sum_j \pi_{ij} = \sum_j \pi_{ji} = \pi_{+i}`$ for all
  $`i`$.
