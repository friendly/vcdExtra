...
for testing structured associations. The visualization methods for these
models were originally developed for models fit using
[`loglm()`](https://rdrr.io/pkg/MASS/man/loglm.html), so this approach
is emphasized here. Some extensions of these methods for models fit
using [`glm()`](https://rdrr.io/r/stats/glm.html) and
[`gnm()`](https://rdrr.io/pkg/gnm/man/gnm.html) are contained in the
`vcdExtra` package and illustrated in .
...
association model can be fit as follows:
