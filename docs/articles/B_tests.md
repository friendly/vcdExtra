...
Generate a crosstable showing cell frequency and the cell contribution
to \\\chi^2\\.
...
generates an error for the `HairEye` data with \\n\\=592 total
frequency.
...
two nominal variables are *conditionally independent*, \\A \perp B \\ \|
\\ C\\, in each stratum, assuming that there is no three-way
interaction. `X` is a 3 dimensional contingency table, where the last
...
frequencies \\n\_{ij}\\ of each \\2 \times 2\\ table are shown as a
quarter circle whose radius is proportional to \\\sqrt{n\_{ij}}\\, so
...
intensely.](fig/tut02-fourfold1-1.png)
...
the factors (shown horizontally). The following call produces , where we
use indexing on the first factor (`Admit`) to make `Admitted` the
highlighted level.
...
given \`Dept\` and \`Gender\`.](fig/tut02-doubledecker-1.png)
...
for the log odds ratio. is produced by:
...
data.](fig/tut02-oddsratio-1.png)
...
tests of association may be obtaianed by taking ordinality into account,
...
Recall the \\4 \times 4\\ table, `JobSat` introduced in @(sec:creating),
...
for contingency tables— similar in spirit to \\r\\ or \\r^2\\ for
...
function. shows the agreementplot for the `SexualFun` data, produced as
shown below.
...
![Agreement plot for the \`SexualFun\` data.](fig/tut02-agreesex-1.png)
...
package gives one implmentation. For an \\r \times c\\ table, the method
...
`ca` object, giving the result in .
...
data](fig/tut02-ca-haireye-1.png)
...
1.  Related methods are the non-parametric CMH tests using assumed
    ...
    [`gnm()`](https://rdrr.io/pkg/gnm/man/gnm.html). Correspondence
    analysis differs in that it is a primarily descriptive/exploratory
    method (no significance tests), but is directly tied to informative
    graphic displays of the row/column categories.
