...
Yet, for analysis, there are time when you need *numeric* values for the
levels of ordered factors in a table, e.g., to treat a factor as a
...
result of a correspondence analysis () below.
...
display of `Treatment` and `Improved` () shows a clearly interpretable
pattern.
...
ignored here.](fig/tut01-Arthritis-1.png)
...
of `UCBAdmissions` in @(sec:mantel) below.[¹](#fn1)
...
in the `plyr` package. For example, with the data in a frequency form
data frame, use [`ddply()`](https://rdrr.io/pkg/plyr/man/ddply.html) to
collapse over unmentioned factors, and
...
[`as.data.frame()`](https://rdrr.io/r/base/as.data.frame.html).[²](#fn2)
...
behavior had been recorded for the Neilson ratings. This data set
...
1.  Changing `Admit` to `Admit?` might be useful for display purposes,
    ...
    name in a model formula. See @(sec:tips) for options `labeling_args`
    and `set_labels`to change variable and level names for displays in
...
3.  [`table()`](https://rdrr.io/r/base/table.html) does not allow a
    ...
    variables are to be found. In the examples in @(sec:table) I used
