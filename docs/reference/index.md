# Package index

## Mosaics

Extensions of [`vcd::mosaic()`](https://rdrr.io/pkg/vcd/man/mosaic.html)
and related displays to `glm` and `gnm` objects and 3D displays

- [`mosaic(`*`<glm>`*`)`](https://friendly.github.io/vcdExtra/reference/mosaic.glm.md)
  [`sieve(`*`<glm>`*`)`](https://friendly.github.io/vcdExtra/reference/mosaic.glm.md)
  [`assoc(`*`<glm>`*`)`](https://friendly.github.io/vcdExtra/reference/mosaic.glm.md)
  : Mosaic plots for fitted generalized linear and generalized nonlinear
  models
- [`mosaic(`*`<glmlist>`*`)`](https://friendly.github.io/vcdExtra/reference/mosaic.glmlist.md)
  [`mosaic(`*`<loglmlist>`*`)`](https://friendly.github.io/vcdExtra/reference/mosaic.glmlist.md)
  : Mosaic Displays for glmlist and loglmlist Objects
- [`glmlist()`](https://friendly.github.io/vcdExtra/reference/glmlist.md)
  [`loglmlist()`](https://friendly.github.io/vcdExtra/reference/glmlist.md)
  [`coef(`*`<glmlist>`*`)`](https://friendly.github.io/vcdExtra/reference/glmlist.md)
  : Create a Model List Object
- [`mosaic3d()`](https://friendly.github.io/vcdExtra/reference/mosaic3d.md)
  : 3D Mosaic Plots
- [`split3d()`](https://friendly.github.io/vcdExtra/reference/split3d.md)
  [`range3d()`](https://friendly.github.io/vcdExtra/reference/split3d.md)
  [`center3d()`](https://friendly.github.io/vcdExtra/reference/split3d.md)
  : Subdivide a 3D Object
- [`seq_mosaic()`](https://friendly.github.io/vcdExtra/reference/seq_mosaic.md)
  : Sequential Mosaics and Strucplots for an N-way Table

## Loglinear models

Functions for constructing and manipulating loglinear models

- [`joint()`](https://friendly.github.io/vcdExtra/reference/loglin-utilities.md)
  [`conditional()`](https://friendly.github.io/vcdExtra/reference/loglin-utilities.md)
  [`mutual()`](https://friendly.github.io/vcdExtra/reference/loglin-utilities.md)
  [`saturated()`](https://friendly.github.io/vcdExtra/reference/loglin-utilities.md)
  [`markov()`](https://friendly.github.io/vcdExtra/reference/loglin-utilities.md)
  [`loglin2formula()`](https://friendly.github.io/vcdExtra/reference/loglin-utilities.md)
  [`loglin2string()`](https://friendly.github.io/vcdExtra/reference/loglin-utilities.md)
  : Loglinear Model Utilities
- [`Crossings()`](https://friendly.github.io/vcdExtra/reference/Crossings.md)
  : Crossings Interaction of Factors
- [`Kway()`](https://friendly.github.io/vcdExtra/reference/Kway.md) :
  Fit All K-way Models in a GLM
- [`glmlist()`](https://friendly.github.io/vcdExtra/reference/glmlist.md)
  [`loglmlist()`](https://friendly.github.io/vcdExtra/reference/glmlist.md)
  [`coef(`*`<glmlist>`*`)`](https://friendly.github.io/vcdExtra/reference/glmlist.md)
  : Create a Model List Object
- [`seq_loglm()`](https://friendly.github.io/vcdExtra/reference/seq_loglm.md)
  : Sequential Loglinear Models for an N-way Table

## Statistical tests

Statistical tests for categorical data

- [`CMHtest()`](https://friendly.github.io/vcdExtra/reference/CMHtest.md)
  [`print(`*`<CMHtest>`*`)`](https://friendly.github.io/vcdExtra/reference/CMHtest.md)
  : Generalized Cochran-Mantel-Haenszel Tests
- [`GKgamma()`](https://friendly.github.io/vcdExtra/reference/GKgamma.md)
  [`print(`*`<GKgamma>`*`)`](https://friendly.github.io/vcdExtra/reference/GKgamma.md)
  : Calculate Goodman-Kruskal Gamma for ordered tables
- [`print(`*`<Kappa>`*`)`](https://friendly.github.io/vcdExtra/reference/print.Kappa.md)
  : Print Kappa
- [`HLtest()`](https://friendly.github.io/vcdExtra/reference/HLtest.md)
  [`print(`*`<HLtest>`*`)`](https://friendly.github.io/vcdExtra/reference/HLtest.md)
  [`summary(`*`<HLtest>`*`)`](https://friendly.github.io/vcdExtra/reference/HLtest.md)
  [`rootogram(`*`<HLtest>`*`)`](https://friendly.github.io/vcdExtra/reference/HLtest.md)
  [`plot(`*`<HLtest>`*`)`](https://friendly.github.io/vcdExtra/reference/HLtest.md)
  : Hosmer-Lemeshow Goodness of Fit Test
- [`logLik(`*`<loglm>`*`)`](https://friendly.github.io/vcdExtra/reference/logLik.loglm.md)
  : Log-Likelihood of a loglm Object
- [`LRstats()`](https://friendly.github.io/vcdExtra/reference/LRstats.md)
  : Brief Summary of Model Fit for glm and loglm Models
- [`modFit()`](https://friendly.github.io/vcdExtra/reference/modFit.md)
  : Brief Summary of Model Fit for a glm or loglm Object
- [`woolf_test()`](https://friendly.github.io/vcdExtra/reference/woolf_test.md)
  [`print(`*`<woolf_test>`*`)`](https://friendly.github.io/vcdExtra/reference/woolf_test.md)
  : Woolf Test for Homogeneity of Odds Ratios
- [`zero.test()`](https://friendly.github.io/vcdExtra/reference/zero.test.md)
  : Score test for zero inflation in Poisson data

## Distributions

Logseries distributions

- [`dlogseries()`](https://friendly.github.io/vcdExtra/reference/logseries.md)
  [`plogseries()`](https://friendly.github.io/vcdExtra/reference/logseries.md)
  [`qlogseries()`](https://friendly.github.io/vcdExtra/reference/logseries.md)
  [`rlogseries()`](https://friendly.github.io/vcdExtra/reference/logseries.md)
  : The Logarithmic Series Distribution

## Utility

Utility functions in the package

- [`datasets()`](https://friendly.github.io/vcdExtra/reference/datasets.md)
  : Information on Data Sets in Packages
- [`blogits()`](https://friendly.github.io/vcdExtra/reference/blogits.md)
  : Bivariate Logits and Log Odds Ratio
- [`collapse.table()`](https://friendly.github.io/vcdExtra/reference/collapse.table.md)
  : Collapse Levels of a Table
- [`cutfac()`](https://friendly.github.io/vcdExtra/reference/cutfac.md)
  : Cut a Numeric Variable to a Factor
- [`expand.dft()`](https://friendly.github.io/vcdExtra/reference/expand.dft.md)
  : Expand a frequency table to case form

## Other

Other functions in the package (not yet classified)

- [`mcaplot()`](https://friendly.github.io/vcdExtra/reference/mcaplot.md)
  : Simple and enhanced plot of MCA solutions

- [`Summarise()`](https://friendly.github.io/vcdExtra/reference/Summarise.md)
  : Brief Summary of Model Fit for glm and loglm Models

- [`update(`*`<xtabs>`*`)`](https://friendly.github.io/vcdExtra/reference/update.xtabs.md)
  :

  Update method for a `xtabs` object

- [`vcdExtra-package`](https://friendly.github.io/vcdExtra/reference/vcdExtra-package.md)
  [`vcdExtra`](https://friendly.github.io/vcdExtra/reference/vcdExtra-package.md)
  [`_PACKAGE`](https://friendly.github.io/vcdExtra/reference/vcdExtra-package.md)
  : Extensions and additions to vcd: Visualizing Categorical Data

## Data

Some small data sets used in examples or in
[DDAR](http://ddar.datavis.ca/)

- [`Abortion`](https://friendly.github.io/vcdExtra/reference/Abortion.md)
  : Abortion Opinion Data
- [`Accident`](https://friendly.github.io/vcdExtra/reference/Accident.md)
  : Traffic Accident Victims in France in 1958
- [`AirCrash`](https://friendly.github.io/vcdExtra/reference/AirCrash.md)
  : Air Crash Data
- [`Alligator`](https://friendly.github.io/vcdExtra/reference/Alligator.md)
  : Alligator Food Choice
- [`Asbestos`](https://friendly.github.io/vcdExtra/reference/Asbestos.md)
  : Effect of Exposure to Asbestos
- [`Bartlett`](https://friendly.github.io/vcdExtra/reference/Bartlett.md)
  : Bartlett Data on Plum Root Cuttings
- [`Burt`](https://friendly.github.io/vcdExtra/reference/Burt.md) :
  Burt (1950) Data on Hair, Eyes, Head and Stature
- [`Caesar`](https://friendly.github.io/vcdExtra/reference/Caesar.md) :
  Risk Factors for Infection in Caesarian Births
- [`Cancer`](https://friendly.github.io/vcdExtra/reference/Cancer.md) :
  Survival of Breast Cancer Patients
- [`Cormorants`](https://friendly.github.io/vcdExtra/reference/Cormorants.md)
  : Advertising Behavior by Males Cormorants
- [`CrabSatellites`](https://friendly.github.io/vcdExtra/reference/CrabSatellites.md)
  : Horseshoe Crab Mating
- [`CyclingDeaths`](https://friendly.github.io/vcdExtra/reference/CyclingDeaths.md)
  : London Cycling Deaths
- [`DaytonSurvey`](https://friendly.github.io/vcdExtra/reference/DaytonSurvey.md)
  : Dayton Student Survey on Substance Use
- [`Depends`](https://friendly.github.io/vcdExtra/reference/Depends.md)
  : Dependencies of R Packages
- [`Detergent`](https://friendly.github.io/vcdExtra/reference/Detergent.md)
  : Detergent Preference Data
- [`Donner`](https://friendly.github.io/vcdExtra/reference/Donner.md) :
  Survival in the Donner Party
- [`Draft1970`](https://friendly.github.io/vcdExtra/reference/Draft1970.md)
  : USA 1970 Draft Lottery Data
- [`Draft1970table`](https://friendly.github.io/vcdExtra/reference/Draft1970table.md)
  : USA 1970 Draft Lottery Table
- [`Dyke`](https://friendly.github.io/vcdExtra/reference/Dyke.md) :
  Sources of Knowledge of Cancer
- [`Fungicide`](https://friendly.github.io/vcdExtra/reference/Fungicide.md)
  : Carcinogenic Effects of a Fungicide
- [`GSS`](https://friendly.github.io/vcdExtra/reference/GSS.md) :
  General Social Survey– Sex and Party affiliation
- [`Geissler`](https://friendly.github.io/vcdExtra/reference/Geissler.md)
  : Geissler's Data on the Human Sex Ratio
- [`Gilby`](https://friendly.github.io/vcdExtra/reference/Gilby.md) :
  Clothing and Intelligence Rating of Children
- [`Glass`](https://friendly.github.io/vcdExtra/reference/Glass.md) :
  British Social Mobility from Glass(1954)
- [`HairEyePlace`](https://friendly.github.io/vcdExtra/reference/HairEyePlace.md)
  : Hair Color and Eye Color in Caithness and Aberdeen
- [`Hauser79`](https://friendly.github.io/vcdExtra/reference/Hauser79.md)
  : Hauser (1979) Data on Social Mobility
- [`Heart`](https://friendly.github.io/vcdExtra/reference/Heart.md) :
  Sex, Occupation and Heart Disease
- [`Heckman`](https://friendly.github.io/vcdExtra/reference/Heckman.md)
  : Labour Force Participation of Married Women 1967-1971
- [`HospVisits`](https://friendly.github.io/vcdExtra/reference/HospVisits.md)
  : Hospital Visits Data
- [`HouseTasks`](https://friendly.github.io/vcdExtra/reference/HouseTasks.md)
  : Household Tasks Performed by Husbands and Wives
- [`Hoyt`](https://friendly.github.io/vcdExtra/reference/Hoyt.md) :
  Minnesota High School Graduates
- [`ICU`](https://friendly.github.io/vcdExtra/reference/ICU.md) : ICU
  data set
- [`JobSat`](https://friendly.github.io/vcdExtra/reference/JobSat.md) :
  Cross-classification of job satisfaction by income
- [`Mammograms`](https://friendly.github.io/vcdExtra/reference/Mammograms.md)
  : Mammogram Ratings
- [`Mental`](https://friendly.github.io/vcdExtra/reference/Mental.md) :
  Mental Impairment and Parents SES
- [`Mice`](https://friendly.github.io/vcdExtra/reference/Mice.md) : Mice
  Depletion Data
- [`Mobility`](https://friendly.github.io/vcdExtra/reference/Mobility.md)
  : Social Mobility data
- [`PhdPubs`](https://friendly.github.io/vcdExtra/reference/PhdPubs.md)
  : Publications of PhD Candidates
- [`ShakeWords`](https://friendly.github.io/vcdExtra/reference/ShakeWords.md)
  : Shakespeare's Word Type Frequencies
- [`TV`](https://friendly.github.io/vcdExtra/reference/TV.md) : TV
  Viewing Data
- [`Titanicp`](https://friendly.github.io/vcdExtra/reference/Titanicp.md)
  : Passengers on the Titanic
- [`Toxaemia`](https://friendly.github.io/vcdExtra/reference/Toxaemia.md)
  : Toxaemia Symptoms in Pregnancy
- [`Vietnam`](https://friendly.github.io/vcdExtra/reference/Vietnam.md)
  : Student Opinion about the Vietnam War
- [`Vote1980`](https://friendly.github.io/vcdExtra/reference/Vote1980.md)
  : Race and Politics in the 1980 Presidential Vote
- [`WorkerSat`](https://friendly.github.io/vcdExtra/reference/WorkerSat.md)
  : Worker Satisfaction Data
- [`Yamaguchi87`](https://friendly.github.io/vcdExtra/reference/Yamaguchi87.md)
  : Occupational Mobility in Three Countries
