...
The current development version (0.9.8) can be installed from
...
apply to loglinear models fitted using
[`MASS::loglm()`](https://rdrr.io/pkg/MASS/man/loglm.html), generalized
linear models using [`stats::glm()`](https://rdrr.io/r/stats/glm.html)
and also to the related, generalized *nonlinear* models fitted with
`gnm()` in the [gnm](https://CRAN.R-project.org/package=gnm) package.
...
- [`vcd::woolf_test()`](https://rdrr.io/pkg/vcd/man/woolf_test.html) is
  extended to provide a decomposition of the overall test for
  homogeneity of odds ratios in 2 x 2 tables when the strata themselves
  represent an R x C table of two other factors

A few others worth knowing about:

- [`breslow_day_test()`](https://friendly.github.io/vcdExtra/reference/breslow_day_test.md)
  is a complementary test for homogeneity of odds ratios across strata
  in 2 x 2 x K tables, with the same `decompose` option as
...
- [`HLtest()`](https://friendly.github.io/vcdExtra/reference/HLtest.md)
  provides the Hosmer-Lemeshow goodness-of-fit test for a binary
  [`glm()`](https://rdrr.io/r/stats/glm.html) model, useful for logistic
  regression models not otherwise well summarized by deviance alone.

- [`zero.test()`](https://friendly.github.io/vcdExtra/reference/zero.test.md)
  is a score test for zero-inflation in Poisson count data, testing
  whether the observed number of zero counts exceeds what a Poisson
  model would predict.
...
- [`drop1.loglm()`](https://friendly.github.io/vcdExtra/reference/drop1.loglm.md),
  ...
  [`MASS::loglm()`](https://rdrr.io/pkg/MASS/man/loglm.html) model, with
  partial R^2 and partial Cramer’s V / Cohen’s w effect-size measures
  for each term.
