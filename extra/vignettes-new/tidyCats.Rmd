...
participation in developing a `tidyCat` package for this purpose.
...
> Tidy categorical data should best be represented as a `tibble` in case form**. A tibble is convenient for its' printing.
...
What is needed are `broom` methods for `loglm` models. The information is accessible from standard functions, but
not in a tidy form.

* `glance.loglm()` -- model level statistics. These are given by There are also related `LRstats()` functions in `vcdExtra`.
...
* `tidy.loglm()` --- coefficient level statistics. These are available from `coef.loglm()`
