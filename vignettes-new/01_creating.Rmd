...
* **case form** 
a data frame containing individual observations, with one or
...
^[`table()` does not allow a `data` argument to provide an environment in which the table variables are to be found.  In the examples in \secref{sec:table} I used `attach(mydata)` for this purpose,
...
```{r tv-mosaic, fig.height=7, fig.width=7}
