...
See \secref{sec:tips} for options `labeling\_args` and `set\_labels`
to change variable and level names 
for displays in the `strucplot` framework.
}
```{r, reorder3,results=verbatim}
...
%
%```{r, reorder4,results=verbatim, keep.source=TRUE}
%class(UCBAdmissions)
%class(UCB)
...
```{r, convert-ex1,results=verbatim}
...
```{r, convert-ex2,results=verbatim}
...
```{r, convert-ex3,results=verbatim}
...
fisher.test(HairEye)
```
%# ```{r, echo=TRUE,eval=FALSE,results=verbatim}
...
```{r, agreesex,fig=TRUE,height=6,width=7,results=verbatim,include=FALSE}
...
%```{r, agreesex,fig=TRUE,height=6,width=7,results=verbatim}
%agree <- agreementplot(SexualFun, main="Is sex fun?")
%agree
%```
