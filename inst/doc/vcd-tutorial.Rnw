...
\subsection{Ordered factors and reordered tables}\label{sec:ordered-factors}
...
See \secref{sec:tips} for options \code{labeling_args} and \code{set_labels}
...
\subsection[Collapsing over table factors]{Collapsing over table factors: \codefun{aggregate} and \codefun{apply}}
...
\subsubsection{Changing the labels for variables and levels}
