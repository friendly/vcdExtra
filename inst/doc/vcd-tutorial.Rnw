...
% Vignette examples
\newcommand*{\Example}{\textbf{\emph{Example}}: }
...
The \pkg{vcdExtra} package extends the graphical methods provided by
\pkg{vcd}.
\end{abstract}
...
models.  The \pkg{vcdExtra} package extends \pkg{vcd} to models fit using
...
\section[Creating tables]{Creating and manipulating frequency tables}\label{sec:creating}
...
with levels 
\Sexpr{paste(levels(Arthritis$Improved),collapse=' < ')}.  Excluding \code{Age}, we would have
...
\subsection[Collapsing]{Collapsing over table factors: \codefun{aggregate} and \codefun{apply}}
...
frequency-form data frame (\codefun{aggregate}), or a table-form array or table object (\codefun{apply}).
...
Dayton.ACM.df <- aggregate(Freq ~ cigarette+alcohol+marijuana, data=DaytonSurvey, FUN=sum)
Dayton.ACM.df
@
...
\subsection[CMH tests: ordinal factors]{Cochran-Mantel-Haenszel tests for ordinal factors}
...
of association, statistically equivalent to the Pearson correlation between the
...
association on 1 df is nearly significant.
...
Goodman-Kruskal $\gamma$ statistic (\citet[\S 2.4.3]{Agresti:2002})
...
The visualization methods for these models was originally developed for models fit using \codefun{loglm},
...
Similarly, the model of \emph{joint independence} allows an association between A and B, but specifies that
...
@
In model comparisons, we can carry out tests of \emph{nested} models with
...
residuals printed in cells).  Note that the formula supplied to \codefun{mosaic}
for \code{glm} objects refers to the order of factors displayed in the plot, not the model.
...
You should read \code{vignette("gnmOverview")} for further details.
...
\section[Continuous predictors]{Continuous predictors: spine and conditional density plots}\label{sec:contin}
