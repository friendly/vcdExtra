...
@
  ...
  is \code{sum(X$Freq)},  \code{sum(X[,"Freq"])} or some equivalent form.
...
The \textbf{Model formula} column shows how to express each model for \codefun{logglm} in \proglang{R}.%
\footnote{
For \codefun{glm}, or \codefun{gnm} with the data in the form of a frequency data.frame, the same model is specified
in the form \code{glm(Freq ~ ..., family="poisson")}.
}
...
The \codefun{glm} approach, and extensions of this in the \pkg{gnm} allows a
...
\3mph{Example}:
The data \code{Mental} gives a two-way table classifying young people by
