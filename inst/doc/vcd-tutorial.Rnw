...
in the form \code{glm(Freq} $\sim$ \code{..., family="poisson")}.
}
...
The \pkg{vcdExtra} provides some additional methods, \codefun{mosaic.glm} and \codefun{mosaic.gnm}
for models fit with \codefun{glm} and \codefun{gnm}.  These are not restricted to the 
